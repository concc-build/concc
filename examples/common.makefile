...
local-build: buildenv | secrets workspace
	...
	docker compose run --rm client concc -C src -p $(PROJECT) -w $(WORKERS) $(BUILD_CMD)
...
remote-build: buildenv | secrets workspace
	...
	docker compose run --rm client concc -C src -l $(CONFIGURE_CMD)
	docker compose run --rm client concc -C src -p $(shell hostname):$(SSH_PORT) -w $(REMOTE_WORKERS) $(BUILD_CMD)

.PHONY: nondist-build
nondist-build: buildenv | secrets workspace
	make src-clean
	docker compose run --rm client concc -C src -l $(NONDIST_CONFIGURE_CMD)
	docker compose run --rm client concc -C src -l $(NONDIST_BUILD_CMD)
