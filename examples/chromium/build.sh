...
do
  ...
  ssh -p $WORKER_PORT $WORKER_HOST chown -R concc:concc /proj
  ssh -p $WORKER_PORT concc@$WORKER_HOST \
  ...
  gosu concc /opt/concc/worker-pool add $WORKER:22 $LIMIT
...
(cd /proj/src; gosu concc gn gen out/Default --args='cc_wrapper="/opt/concc/concc-wrapper"')

echo "Building $TARGET with worker containers..."
(cd /proj/src; gosu concc autoninja -C out/Default -j $JOBS $TARGET)
